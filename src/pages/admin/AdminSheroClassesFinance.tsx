...
import {
  generateSheroClassesVouchers, generateSheroClassesPL, generateReceivablesPayables,
...
const generateMonthlyData = () => months.map(month => {
  const sessions = Math.floor(60 + Math.random() * 80);
  const rev = Math.round(sessions * 550); const exp = Math.round(rev * 0.48);
  const cm1 = rev - exp; const oh = Math.round(rev * 0.08); const cm15 = cm1 - oh;
  return { month, sessions, revenue: rev, expenditure: exp, cm1, cm15, cm1Pct: Math.round(cm1/rev*1000)/10 };
});

export default function AdminSheroClassesFinance() {
  const role = getAdminRole();
  const canDownload = !role || ["super_admin","country_manager","vertical_head","finance_manager"].includes(role);
  const [period, setPeriod] = useState("mtd");
  const [tab, setTab] = useState("pl");

  const vouchers = useMemo(() => generateSheroClassesVouchers(), []);
  const plStatement = useMemo(() => generateSheroClassesPL(), []);
  const receivablesPayables = useMemo(() => generateReceivablesPayables("shero_classes"), []);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">🧘 Shero Classes Finance</h1>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="pl" className="mt-4">
          <Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><BookOpen className="w-4 h-4 text-primary" /> P&L Statement — Shero Classes (Yoga & Wellness)</CardTitle></CardHeader>
