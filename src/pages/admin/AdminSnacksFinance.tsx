...
import {
  IndianRupee, TrendingUp, Download, BookOpen, FileText,
  CheckCircle2, Clock, Receipt, Wallet, Scale,
} from "lucide-react";
import { getAdminRole } from "@/data/adminRoles";
import {
  generateSnacksVouchers, generateSnacksPL, generateReceivablesPayables,
  aggregateLedger, generateTrialBalance, generateDayBook, generateCashBankBook,
  generateSalesRegister, generatePurchaseRegister,
...
const generateMonthlyData = () => months.map(month => {
  const orders = Math.floor(150 + Math.random() * 200);
  const rev = Math.round(orders * 450); const exp = Math.round(rev * 0.73);
  const cm1 = rev - exp; const oh = Math.round(rev * 0.08); const cm15 = cm1 - oh;
  return { month, orders, revenue: rev, expenditure: exp, cm1, cm15 };
});

export default function AdminSnacksFinance() {
  const role = getAdminRole();
  const canDownload = !role || ["super_admin","country_manager","vertical_head","finance_manager"].includes(role);
  const [period, setPeriod] = useState("mtd");
  const [tab, setTab] = useState("pl");

  const vouchers = useMemo(() => generateSnacksVouchers(), []);
  const plStatement = useMemo(() => generateSnacksPL(), []);
  const receivablesPayables = useMemo(() => generateReceivablesPayables("snacks"), []);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">🍬 Sweets & Snacks Finance</h1>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="pl" className="mt-4">
          <Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><BookOpen className="w-4 h-4 text-primary" /> P&L Statement — Sweets & Snacks</CardTitle></CardHeader>
          <CardContent><div className="space-y-1">
            {plStatement.map((row, i) => (
              <div key={i} className={`flex justify-between items-center py-1 ${row.type === "header" && row.label ? "border-t border-border pt-2 mt-2" : ""} ${row.indent ? "pl-6" : ""}`}>
                ...
                {row.label && <span className={`font-mono text-sm ${row.bold ? "font-bold" : ""} ${row.amount < 0 ? "text-destructive" : row.type === "subtotal" && row.bold ? "text-primary" : "text-foreground"}`}>{row.amount !== 0 ? (typeof row.amount === "number" && row.label.includes("%") ? `${row.amount}%` : fmtSigned(row.amount)) : ""}</span>}
              </div>
            ))}
        ...
        <TabsContent value="trial" className="mt-4">
          <Card><CardContent className="p-0">
            <Table><TableHeader><TableRow>
              <TableHead className="text-xs">Ledger Account</TableHead><TableHead className="text-xs">Group</TableHead>
              <TableHead className="text-xs text-right">Debit</TableHead><TableHead className="text-xs text-right">Credit</TableHead>
            </TableRow></TableHeader><TableBody>
              {trialBalance.filter(r => r.debit > 0 || r.credit > 0).map(r => (
                <TableRow key={r.account}><TableCell className="text-sm">{r.label}</TableCell><TableCell><Badge variant="outline" className="text-[10px]">{ledgerGroupLabels[r.group]}</Badge></TableCell>
                  <TableCell className="text-xs text-right font-mono">{r.debit > 0 ? fmtFull(r.debit) : "—"}</TableCell>
                  <TableCell className="text-xs text-right font-mono">{r.credit > 0 ? fmtFull(r.credit) : "—"}</TableCell>
                </TableRow>))}
              <TableRow className="bg-muted/30 font-bold"><TableCell colSpan={2} className="text-sm">TOTAL</TableCell>
                <TableCell className="text-xs text-right font-mono">{fmtFull(tbDebitTotal)}</TableCell>
                <TableCell className="text-xs text-right font-mono">{fmtFull(tbCreditTotal)}</TableCell>
              </TableRow>
            </TableBody></Table>
          </CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="daybook" className="mt-4">
          <Card><CardContent className="p-0">
            <Table><TableHeader><TableRow>
              <TableHead className="text-xs">Date</TableHead><TableHead className="text-xs">Voucher</TableHead><TableHead className="text-xs">Type</TableHead>
              <TableHead className="text-xs">Party</TableHead><TableHead className="text-xs">Narration</TableHead>
              <TableHead className="text-xs text-right">Debit</TableHead><TableHead className="text-xs text-right">Credit</TableHead>
            </TableRow></TableHeader><TableBody>
              {dayBook.map(d => (<TableRow key={d.voucherId}><TableCell className="text-xs">{d.date}</TableCell><TableCell className="text-xs font-mono">{d.voucherId}</TableCell>
                <TableCell><Badge className={`text-[10px] ${voucherTypeLabels[d.voucherType].color}`}>{voucherTypeLabels[d.voucherType].shortCode}</Badge></TableCell>
                <TableCell className="text-xs">{d.partyName}</TableCell><TableCell className="text-xs max-w-[200px] truncate">{d.narration}</TableCell>
                <TableCell className="text-xs text-right font-mono">{fmtFull(d.debit)}</TableCell><TableCell className="text-xs text-right font-mono">{fmtFull(d.credit)}</TableCell>
              </TableRow>))}
            </TableBody></Table>
          </CardContent></Card>
        </TabsContent>
          ...
          {vouchers.map(v => (
            <Card key={v.id}><CardContent className="pt-4">
              <div className="flex items-center gap-2 mb-2">
                <Badge className={`text-[10px] ${voucherTypeLabels[v.type].color}`}>{voucherTypeLabels[v.type].label}</Badge>
