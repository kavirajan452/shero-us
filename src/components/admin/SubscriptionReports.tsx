...
const SubscriptionReports = () => {
  ...
  const handleExport = () => {
    toast({ title: "📥 Report Downloaded", description: `subscription_report_${period}.xlsx` });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="outline" className="text-[10px] h-7 gap-1" onClick={handleExport}>
      ...
      <Card className="border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-xs flex items-center gap-1.5">
            <Package className="w-3.5 h-3.5 text-primary" /> Plan-wise Performance
          </CardTitle>
        </CardHeader>
              ...
              <TableBody>
                {planBreakdown.map(p => (
                  <TableRow key={p.plan}>
                    ...
                    <TableCell className="text-[10px]">
                      <Badge className={`text-[8px] ${p.onTime >= 90 ? "bg-green-100 text-green-800" : p.onTime >= 85 ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"}`}>
                        {p.onTime}%
                      </Badge>
                    </TableCell>
      ...
      <Card className="border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-xs flex items-center gap-1.5">
            <ChefHat className="w-3.5 h-3.5 text-primary" /> Partner Performance
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="overflow-x-auto">
      ...
      {/* ── Skip Pattern Heatmap ── */}
      <Card className="border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-xs flex items-center gap-1.5">
            <SkipForward className="w-3.5 h-3.5 text-primary" /> Weekly Skip Pattern
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
          ...
          <p className="text-[9px] text-muted-foreground mt-2">
            💡 <strong>Insight:</strong> Saturday has highest skip rates across all slots. Consider flexible Saturday menus or weekend pause option.
          </p>
        </CardContent>
      </Card>
      ...
      <Card className="border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-xs flex items-center gap-1.5">
            <TrendingUp className="w-3.5 h-3.5 text-primary" /> Retention Funnel (This Month)
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          ...
          <p className="text-[9px] text-muted-foreground mt-1">
            💡 <strong>Insight:</strong> 64.6% trial-to-weekly conversion is strong. Focus on weekly-to-monthly retention (43.1%) — consider loyalty discounts at day 5.
          </p>
        </CardContent>
      </Card>
    </div>
